...
# Detect if running headless (Docker) or with NO_GUI flag
HEADLESS = os.environ.get("NO_GUI") or os.environ.get("DISPLAY") is None
...
@app.route("/")
def home():
    return "Welcome to ACEest Fitness App! Running in {} mode.".format(
        "headless" if HEADLESS else "GUI"
    )

if not HEADLESS:
    # Import Tkinter only if GUI is available
    ...
    class ACEestApp:
        ...
        def setup_ui(self):
            ...
            header.pack(fill="x")
            tk.Label(header, text="ACEest FUNCTIONAL FITNESS",
                     font=("Helvetica", 24, "bold"),
                     bg="#d4af37", fg="black").pack(pady=20)
            ...
            left_panel = tk.LabelFrame(main_frame, text=" Client Profile ",
                                       fg="#d4af37", bg="#1a1a1a",
                                       font=("Arial", 12, "bold"))
            left_panel.pack(side="left", fill="y", padx=10)

            tk.Label(left_panel, text="Select Program:", bg="#1a1a1a",
                     fg="white").pack(pady=10)
            self.prog_var = tk.StringVar()
            self.prog_menu = ttk.Combobox(left_panel,
            ...
            tk.Label(left_panel, text=metrics_text, bg="#333", fg="#ddd",
                     font=("Courier", 10), justify="left").pack(side="bottom", fill="x", pady=20)
            ...
            # Workout Chart
            self.work_frame = tk.LabelFrame(self.right_panel, text=" Weekly Workout Chart ",
                                            fg="#d4af37", bg="#1a1a1a", font=("Arial", 12))
            self.work_frame.pack(fill="both", expand=True, pady=5)
            self.work_label = tk.Label(self.work_frame,
    ...
    print("Running in headless mode (Docker or server)")